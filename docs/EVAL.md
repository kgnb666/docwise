...
- 语料：`backend/samples/*.md`（10 篇技术笔记，40 块，含主题重叠文档制造检索难度）
- 测试集：`backend/tests/data/test_set.json`，32 条 QA（含改写题/精确术语题/跨主题易混淆题）
- 扩充方式：用户真实知识库内容进来后，按同样格式继续加
...
```
语料 40 块 | 测试集 32 条 | top-k=1 | 分词 jieba
纯向量检索                  31/32  96.9%
混合检索                    32/32  100.0%
...
### 4.1.1 一个真实的对比案例（面试可讲）

top-k=1 时纯向量检索漏掉的题：**「Transformer 为什么是大模型的基石」**
- 纯向量被 `llm-notes.md`（大模型笔记）带偏——它俩都提到 Transformer/大模型，语义相近；
- **混合检索靠 BM25 的精确关键词命中 `deep-learning-notes.md` 修正了结果**；
- bge-reranker（交叉编码）同样正确重排。
- 结论：语义相近的跨主题问题上，关键词信号是重要的纠偏器——这正是混合检索的价值。

### 4.2 生成质量（LLM-as-Judge：DeepSeek 判分，32 题实测）
